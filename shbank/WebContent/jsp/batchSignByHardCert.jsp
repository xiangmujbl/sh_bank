...
<script data-main="<%=request.getContextPath()%>/resources/js/drecord.js" src="<%=request.getContextPath()%>/resources/js/require.js"></script>
...
//单个签署
function singleSign(data,orderId)
{
	selectCert(data,orderId);
}
</script>
</head>
...
<div class="container-full">
  <div class="content">
    ...
    <form action="<%=request.getContextPath()%>/batchSign.do" id="form1" name="form1">
    <div class="search">
      ...
      <button type="button" class="btn" disabled id="sign">批量签署</button>
     
    </div>
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="data">
      <tbody>
        <tr>
          <th class="center" scope="col"><input type="checkbox" class="checkall" title="全选"></th>
        ...
        %>
          ...
          <td><i class="icon icon-chevron-circle-down">&#xf13a;</i>
            ...
            <%if("0".equals(signSta)&&!"3".equals(contract.getStatus())&&!"4".equals(contract.getStatus())&&!"5".equals(contract.getStatus())){
            	...
            	%>
            	<td><div class="selectbox"> <a href="javascript:singleSign('<%=contract.getSignPlaintext()%>','<%=contract.getOrderId()%>')" class="selected" 
="<%=contract.getSignPlaintext()%>"  serialNum="<%=contract.getOrderId()%>">立即签署 <i class="icon icon-angle-down"></i></a>
              <ul class="options">
              <%if(creatorPlatformUserName.equals(userId)){
            	  %><li>
            	  <a href="javascript:cancelContarct('<%=appId %>','<%=time %>','<%=sign %>','<%=signType %>','<%=userId %>','<%=orderId %>');" serialNum="<%=contract.getOrderId()%>">
            	  撤销签署</a>
            	  </li><% 
              }else{
            	  %><li>
                ...
                <li>
                <a href="javascript:showContract('<%=appId %>','<%= time%>','<%= sign%>','<%=signType %>','<%=userId %>','<%= orderId%>');" serialNum="<%=contract.getOrderId()%>">
                	查看合同</a>
                </li>
              </ul>
            </div></td>
...
<form action="" id="signform" name ="signform" method="post" style="display:none;">
		<input hidden name='cert' />
		<input hidden name='data' />
		<input hidden name='sign' />
		<input hidden name='xlh' />
		<input hidden name='t' />
		<input hidden name="serial_nums"/>
		<input hidden id="appId" name="appId" value="${appId}"/>
		<input hidden id="ucid" name="ucid" value="${userId}"/>
		<input  hidden name='sealBase64' />
 </form>
