...
public class FtpDownloadTest extends HttpServlet {
	...
	/*	ServiceClient serviceClient = new ServiceClient();
        ...
        //设置命名空间
        OMNamespace omNs = fac.createOMNamespace("http://wsdl.com/","ftpDownload");
        OMElement eleData = fac.createOMElement("ftpDownload", omNs);
        //对应参数的节点
        String[] strs = new String[] { "appId", "time", "sign", "signType", "userId","orderList" };
        //参数值
        String[] val = new String[] {appId, time, sign, signType, userId, orderList};
        ...
        while(iterator.hasNext()){
		 ...
		 String[] paramName = new String[] { "appId", "time", "sign", "signType", "userId","orderList" };
		    
	        String[] paramValue = new String[] {appId, time, sign, signType, userId, orderList};

			String result = CallWebServiceUtil.CallHttpsService(endpoint, "http://wsdl.com/", "ftpDownload", paramName, paramValue);
...
//			call.setOperationName(new QName("http://wsdl.com/", "ftpDownload"));
//
...
//			call.addParameter("orderList", XMLType.XSD_STRING, ParameterMode.IN);
//			call.setReturnType(XMLType.XSD_STRING);
//			String result = call.invoke(new Object[] { appId, time, sign, signType, userId, orderList }).toString();
//			System.out.println(result);
//			request.getSession().setAttribute("result", result);
//			response.sendRedirect("jsp/result.jsp");// 跳转至结果页
