...
<body>
<form action="<%=request.getContextPath() %>/customLogo" method="post" enctype="multipart/form-data">
  <table class="data-box" width="80%" >
  <thead>
    <tr id="proTr">
      <th colspan="3">自定义logo接口演示</th>
    </tr>
    </thead>
    <tbody>
    ...
    <tr id="widthTr">
      <td>width</td>
      <td>图片宽度,最大为400</td>
      <td><input type="text" id="width" name="width" value="" size="50" /></td>
    </tr>
    <tr id="heightTr">
      <td>height</td>
      <td>图片高度,最大为70</td>
      <td><input type="text" id="height" name="height" value="" size="50" /></td>
    </tr>
    ...
    <tr id="imageTr">
	    <td>image</td>
        <td>上传logo图片，最大为2m</td>
        <td>
            <div class="upload_recive clear">
				<input type="file" name="image" class="image" id="image" data-ext="jpg,jpeg"/>
        	</div>
        </td>
    </tr>
		...
		$(function(){
			...
			$('.image').change(function(){
				...
				file_ext=fileext(filepath);
				 ...
				 else if($.inArray(file_ext,arr_ext)>-1 || file_ext=='' ){
					return true;
				}
				else{
