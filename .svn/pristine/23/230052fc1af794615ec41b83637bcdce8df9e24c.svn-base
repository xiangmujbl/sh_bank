...
public class RegisterWithOCRTest extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try {
				...
				while (i.hasNext()) {
					...
					if (!fi.isFormField()) {
						...
						} else if (fi.getFieldName().equals("isBusinessAdmin")) {
							isBusinessAdmin = fi.getString();
		...
		// md5拼接，校验
		String md5str = appId + "&" + identityCard + "&"+ time+ "&" + userName + "&" + userId;
		...
		//使用ServiceClient调用https的WebService
			/*	ServiceClient serviceClient = new ServiceClient();
		        ...
		        //设置命名空间
		        OMNamespace omNs = fac.createOMNamespace("http://wsdl.com/","registerWithOCR");
		        OMElement eleData = fac.createOMElement("registerWithOCR", omNs);
		        //对应参数的节点
		 ...
		 String[] paramName = new String[] { "appId", "time", "sign", "signType", "info" };
			...
			String result = CallWebServiceUtil.CallHttpsService(endpoint, "http://wsdl.com/", "registerWithOCR", paramName, paramValue);
...
//			call.setOperationName(new QName("http://wsdl.com/", "registerWithOCR"));
//
