...
<body>
<form action="<%=request.getContextPath() %>/syncOperateStatusTest" method="post">
  <table class="data-box" width="80%" >
  <thead>
    <tr id="proTr">
      <th colspan="3">同步状态接口演示</th>
    </tr>
    </thead>
    <tbody>
    ...
    <tr id="syncTypeTr">
      <td>syncType</td>
      <td> 同步类型：1、签署状态同步；2、手机号变更同步</td>
      <td>
      	<select id="syncType" name="syncType">
      		<option value="1">签署状态同步</option>
      		<option value="2">手机号变更同步</option>
      	</select>
      </td>
    </tr>
